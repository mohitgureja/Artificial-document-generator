...
class Drawtable:
    ...
    def __init__(self, data, x, y, font=None, font_size=16, drawsheet=None, xend=None, line_spacer=20, margin_text=10,
                 line_width=2, return_params=False, **kwargs):

        image_width, image_height = 0, 0
        if font is None:
            font = PIL.ImageFont.truetype("assets/arial.ttf", font_size)
        if type(font) != PIL.ImageFont.FreeTypeFont: raise ValueError(
            "Font type expected to be Pillow Font eg : h2_font=   ImageFont.truetype('TimesNewRoman/times new roman.ttf', font_size)'")
        ...
        if drawsheet is not None:
            ...
            if self.columns_width is not None:
                if isinstance(self.columns_width, list) and sum(self.columns_width) == 1:
                    ...
                    raise ValueError("columnwidth must be list eg: [0.1,0.9] and sum must be 1")
    ...
    def __draw_text(self, xy, text="text", font=None, fill=None):
        ...
        return self.__draw.text(xy=xy, text=f"{text}", fill=fill, font=font, anchor=self.anchor, spacing=self.spacing,
                                align=self.align, stroke_width=self.stroke_width, stroke_fill=self.stroke_fill)
    ...
    def draw_table(self):
        try:
            ...
            for row_idx, row in enumerate(self.data):
                h_c = []
                if row_idx == 0 and self.outer_frame: self.__draw_line(self.x_init,
                                                                       self.y_rate - int(self.line_spacer // 2),
                                                                       self.xend, self.y_rate - int(
                        self.line_spacer // 2))  # horizontal
                ...
                if (self.inner_frame or self.rows_frame) and row_idx != len(self.data) - 1: self.__draw_line(
                    self.x_init, self.y_rate - int(self.line_spacer // 2), self.xend,
                                 self.y_rate - int(self.line_spacer // 2))  # horizontal
                if self.header_frame and row_idx == 0: self.__draw_line(self.x_init,
                                                                        self.y_rate - int(self.line_spacer // 2),
                                                                        self.xend, self.y_rate - int(
                        self.line_spacer // 2))  # horizontal
                if row_idx == len(self.data) - 1 and self.outer_frame:
                    self.__draw_line(self.x_init, self.y_rate - int(self.line_spacer // 2), self.xend,
                                     self.y_rate - int(self.line_spacer // 2))  # horizontal
            ...
            if self.inner_frame or self.columns_frame:
                for col in range(self.number_of_columns + 1):
                    if (col == 0 or col == self.number_of_columns) and self.outer_frame == False:
                        continue
                    else:
                        self.__draw_line(self.x_init + int(sum(self.__width_per_cell[:col])),
                                         self.y_init - int(self.line_spacer // 2),
                                         self.x_init + +int(sum(self.__width_per_cell[:col])),
                                         self.y_rate - int(self.line_spacer // 2))
            ...
            if self.new_img:
                if self.save is not None:
                    if str(self.save).find('.') == -1:
                        save = self.save + '.png'
                    else:
                        save = self.save
                    self.__test_back.save(save)
                    print(f"Image saved as {save}")
                return self.__test_back, self.x_init, self.y_init, self.xend, self.y_rate - int(self.line_spacer // 2)
